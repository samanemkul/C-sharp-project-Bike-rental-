    ...
    <div class="container px-4 px-lg-5 mt-5">
        <div class="form-group">
            <div class="input-group mb-3">
                <input type="text" id="search" class="form-control" placeholder="Search Category Here" aria-label="Search Category Here" aria-describedby="search-group">
                <div class="input-group-append">
                    <span class="input-group-text" id="search-group"><i class="fa fa-search"></i></span>
                </div>
            </div>
        </div>
        <div id="category-list" class="row gx-2 gx-lg-5 row-cols-1 row-cols-md-2 row-cols-xl-2 justify-content-center">
            ...
            <div class="col mb-6 mb-2 ">
                <a href="./?p=bikes&c=<?php echo md5($row['id']) ?>" class="card category-item text-dark text-decoration-none shadow">
...
<script>
    $(function(){
        $('#search').on('input',function(e){
            e.preventDefault()
            var _find = $(this).val().toLowerCase()
            $('#category-list .col').each(function(){
                if(($(this).text().toLowerCase().trim()).includes(_find) == true){
                    $(this).toggle(true)
                }else{
                    $(this).toggle(false)
                }
