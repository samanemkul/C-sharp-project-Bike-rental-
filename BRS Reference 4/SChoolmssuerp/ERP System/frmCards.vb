...
Public Class frmCards
        ...
        Try
            ...
            For i As Integer = listView1.Items.Count - 1 To 0 Step -1
                ...
                Else
                    Status = "No"
                End If
                Dim cd As String = "update Cards_Student set Status= '" & Status & "' where AdmissionNo=@d1"
    ...
    Private Sub btnGetData_Click(sender As System.Object, e As System.EventArgs) Handles btnSearch.Click
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("select Student.AdmissionNo,StudentName,Cards_Student.Status from Student,Class,Section,SchoolInfo,Cards_Student where Student.SectionID=Section.ID and Class.ClassName=Section.Class and SchoolInfo.S_ID=Student.SchoolID and Student.AdmissionNo=Cards_Student.AdmissionNo and Session=@d1 and ClassName=@d2 and SectionName=@d3 and Student.Status='Active' order by StudentName", con)
            cmd.Parameters.AddWithValue("@d1", cmbSession.Text)
            cmd.Parameters.AddWithValue("@d2", cmbClass.Text)
            cmd.Parameters.AddWithValue("@d3", cmbSection.Text)
            rdr = cmd.ExecuteReader()
    ...
    Private Sub Button5_Click(sender As System.Object, e As System.EventArgs) Handles Button5.Click
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("select St_ID,Staff.StaffID,StaffName,Cards_Staff.Status from Staff,Cards_Staff,Department,Staff_Department where Staff.St_ID=Cards_Staff.StaffID and Staff.St_ID=Staff_Department.StaffID and Department.ID=Staff_Department.DepartmentID and DepartmentName=@d1 and Staff.Status='Active' order by StaffName", con)
        ...
        Try
            ...
            For i As Integer = ListView2.Items.Count - 1 To 0 Step -1
                ...
                Else
                    Status = "No"
                End If
                Dim cd As String = "update Cards_Staff set Status= '" & Status & "' where StaffID=@d1"
                cmd = New SqlCommand(cd)
